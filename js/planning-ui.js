...
function creerCreneauRepas(date, moment) {
    const repas = trouverRepas(date, moment);
    const titreMoment = moment === "midi" ? "Midi" : "Soir";
    let contenu = "";
        ...
        if (repas.recette_id) {
            contenu = `
                <div class="repas-planifie">
                    <a
                        href="recette.html?id=${encodeURIComponent(repas.recette_id)}"
...
function afficherSemaine() {
    ...
    numeroSemaine.textContent =
        `Semaine ${obtenirNumeroSemaine(debutSemaine)}`;

    titreSemaine.textContent =
        `${formaterDateCourte(debutSemaine)} – ${finSemaine.toLocaleDateString(
            "fr-FR",
    ...
    for (let index = 0; index < 7; index++) {
        ...
        jours.push(`
            <article
                class="jour-planning ${estAujourdhui(date) ? "aujourdhui" : ""}"
            >
                ...
                ${creerCreneauRepas(date, "midi")}
                ${creerCreneauRepas(date, "soir")}
            </article>
        `);
    }

    grilleSemaine.innerHTML = jours.join("");
}
...
function afficherResultatsRecettes(recherche) {
    const texte = recherche.trim().toLowerCase();

    const recettesFiltrees = recettes
        .filter(function (recette) {
            return recette.nom.toLowerCase().includes(texte);
        })
        .slice(0, 12);
    ...
    resultatsRecettes.innerHTML = recettesFiltrees
